...
- 2026-04-25 - Production-quality hardening pass: fixed 4 broken internal exhibit links (`redType-a` → `red-type-a`, `twosquare`/`foursquare` ↔ `two-square`/`four-square`, kryha → m209) and resolved 114 WCAG heading-skip violations (h1→h3 / h2→h4) across 111 pages by promoting first-section and footer headings to the correct level. Added `tests/test-structural.js` (862 assertions covering duplicate IDs, internal-link integrity, heading hierarchy, single `<main>` landmark, `target="_blank"` noopener safety, and CSS focus-visible / reduced-motion presence) and wired it into `npm test`. All 7 suites now green: 435 + 238 + 1836 + 1720 + 519 + 783 + 862 = 6,393 assertions, 0 failures.
