...
35 of 38 cipher pages have fully interactive encrypt/decrypt demos built with vanilla JavaScript — no frameworks, no build tools. Type a message, set a key, and watch the cipher work in real time. Demos are dynamically generated by `demo-loader.js` from cipher engine implementations in `all-engines.js`.
